    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                original = self.ant
                if original.can_contain(insect):
                    original.contain_ant(insect)
                elif insect.can_contain(original):
                    insect.contain_ant(original)
                    self.ant = insect
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                elif not hasattr(self.ant, 'impostor') or (self.ant.name == 'Queen' and self.ant.impostor):
                    self.ant = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if not hasattr(self.ant.ant, 'impostor') or (self.ant.ant.name == 'Queen' and self.ant.ant.impostor):
                        self.ant.ant = None
                else:
        ...
        if not hasattr(insect, 'impostor') or (insect.name == 'Queen' and insect.impostor):
            insect.place = None
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        insect = self.place.ant
        if insect is not None:
            return insect.blocks_path
        return False
        # END Problem 6A
    ...
    def checkTimer(self):
        if self.slowed_timer == 0 and self.stunned_timer == 0:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while len(temp_place.bees) == 0 and temp_place.entrance != hive:
...
class ShortThrower(ThrowerAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    def nearest_bee(self, hive):
        temp_place, max_counter = self.place, 0
        while len(temp_place.bees) == 0 and temp_place.entrance != hive and max_counter < self.max_range:
            temp_place = temp_place.entrance
            max_counter += 1
        return random_or_none(temp_place.bees)
    # END Problem 4B
...
class QueenAnt(ThrowerAnt):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            super().action(colony)
            self.double_minions_damage()
        # END Problem 9

    def double_minions_damage(self):
        temp_place = self.place.exit
        while temp_place != None:
            if temp_place.ant != None:
                if temp_place.ant.container and temp_place.ant.ant != None:
                    if temp_place.ant.ant not in self.buffed_minions:
                        temp_place.ant.ant.damage *= 2
                        self.buffed_minions.append(temp_place.ant.ant)
                if temp_place.ant not in self.buffed_minions:
                    temp_place.ant.damage *= 2
                    self.buffed_minions.append(temp_place.ant)
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def affected_action(self):
        def afffected_action(colony):
            if colony.time % 2 == 0:
                action(colony)
            self.slowed_timer -= 1
            self.checkTimer()
        return afffected_action

    return affected_action(action.__self__)
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    if effect.__name__ == 'make_slow':
        bee.slowed_timer += duration
    if effect.__name__ == 'make_stun':
        bee.stunned_timer += duration
