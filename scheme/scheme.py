...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        rest_evaled = rest.map((lambda item: scheme_eval(item, env)))
        return scheme_apply(scheme_eval(first, env), rest_evaled, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Mismatch length of formals and vals')
        while formals != nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lambda_exp = Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(lambda_exp, env))
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    symbols, exprs = nil, nil
    while bindings != nil:
        check_form(bindings.first, 2, 2)
        symbols = Pair(bindings.first.first, symbols)
        exprs = Pair(scheme_eval(bindings.first.second.first, env), exprs)
        bindings = bindings.second
    check_formals(symbols)
    frame = env.make_child_frame(symbols, exprs)
    return frame
    # END PROBLEM 15
