...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min (centroids, key = lambda k: distance(k, location))
    # END Question 3
...
def find_centroid(cluster):
    ...
    mean_long = mean([restaurant_location(r)[1] for r in cluster])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    d = {find_predictor(user, reviewed, f)[0]:find_predictor(user, reviewed, f)[1] for f in feature_fns}
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    d1 = {restaurant_name(r):user_rating(user,restaurant_name(r)) for r in reviewed}
    not_reviewed = [r for r in restaurants if r not in reviewed]
    d2 = {restaurant_name(r):predictor(r) for r in not_reviewed}
    d2.update(d1)
    return d2
    # END Question 9
